	...
	)
{
	...
	for( register int j=0; j<count_Mtch; j++)
	{
		...
		for( register int i=0; i<count_MtchErr; i++)
		{
			...
			tmpix = spTmp._A*rx+spTmp._B*ry+spTmp._C;
			tmpiy = spTmp._D*rx+spTmp._E*ry+spTmp._F;
	...
	)
{
	...
	{
		ROS_ERROR( "Cann't Read Height Image!" );
		return;
	}

	if( ipMatching.size()!=0 )
		ipMatching.clear();
	...
	if( ipMatching.size()>=3 )
		...
		ROS_ERROR( "less than 3 matching." );
}
